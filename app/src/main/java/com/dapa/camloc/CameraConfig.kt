...
class CameraConfig(context: Context) {
    lateinit var cameraParams: List<CameraParams>

    init {
        val providerFuture = ProcessCameraProvider.getInstance(context)
        providerFuture.addListener({
            val provider = providerFuture.get()
            cameraParams = provider.availableCameraInfos.map { getCameraParams(context, it) }
    ...
    companion object {
        ...
        fun getCameraParams(context:Context, cameraInfo: CameraInfo): CameraParams {
